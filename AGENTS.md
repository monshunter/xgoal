...
当前仓库已实现 v0.1 的自然语言 Planner、SQLite 单一状态、Unix Socket CLI/API、Codex/Claude Agent Profile、当前主目录串行执行与私有 index/Patch、受信 Validator/Evidence、独立 Review、可选 Acceptance、服务与场景生命周期、Gate/Reconcile、串行 Promotion、崩溃恢复、实时 Invocation 观测、一致审计导出、Final Report 与固定 Benchmark Harness。v0.1 只面向 macOS/Linux 上的可信本地 Git 仓库，默认串行执行和 L0 本地进程隔离，不承诺容器级安全，不自动 push、发布或部署生产。
