...
func TestLiveStderrAndSlowNotificationCannotBlockCancellation(t *testing.T) {
	...
	data = []byte(strings.Replace(string(data), "if [ \"$XGOAL_FIXTURE_MODE\" = \"blocking\" ]; then", "if [ \"$XGOAL_FIXTURE_MODE\" = \"blocking\" ]; then\n  printf 'api_key=live-sentinel\\nvisible diagnostic\\n' >&2", 1))
	if err := os.WriteFile(f.binaryPath, data, 0700); err != nil {
		t.Fatal(err)
	}
	runtime := f.adapter(t)
	...
	live := filepath.Join(f.runtimeRoot, "adapters", "codex", "invocations", "invocation_live", "stderr", "events", "000002.json")
