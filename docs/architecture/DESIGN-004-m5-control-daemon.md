...
3. 未知 token/费用不是零；硬预算阻止新调度，但不能把未验收工作变成完成。
...
CLI ── HTTP/JSON over 0600 Unix Socket ── API
          ...
          ┌─────────┼──────────┬───────────────┐
          ▼         ▼          ▼               ▼
      Reconcile   Policy      Budget        Recovery
          └─────────┴──────────┴───────────────┘
                            │
                         SQLite
```
...
- `internal/budget`：typed limit/usage、unknown 语义、soft/hard 预检；不推断供应商未报告数据。
- `internal/store/sqlite`：Gate 授权消费、Budget ledger、Failure/Decision、Worker 和全局 Event 游标的唯一持久 owner。
...
决策输入包含 Failure、当前/前一 Snapshot、相同 Fingerprint + Strategy 的历史次数、Policy/Budget 状态。输出只允许 `RETRY_NEW_ATTEMPT`、`DIAGNOSE`、`FIX_WORK_ITEM`、`SWITCH_STRATEGY`、`REPLAN`、`WAIT_GATE`、`WAIT_BUDGET`、`QUARANTINE`、`STOP_INVARIANT`。重复指纹且无实质进展时优先选择非重试动作；Replan 保存旧 Revision 与影响分析，改变 Goal Contract 时转 Gate。
...
预算维度使用稳定 key；数值用整数最小单位（毫秒、token、微货币单位、字节），避免浮点累计。Usage 值包含 `known`，unknown 不能参与“尚有零消耗”的推断：

- 可确定的离散/本地维度按 persisted consumed + requested 做 preflight。
- token/费用未知且存在对应硬限额时 fail closed，返回 `REQUIRE_USAGE`/`HARD_BLOCK`，不能按零放行。
- soft limit 首次越过时追加 Attention Event；hard limit 阻止新的 Attempt/Probe。
- 结束后用供应商实际 Usage 记账；预算扩额只接受匹配的已消费 Gate 授权。
...
- `budget_limits` / `budget_usage`：Goal 与可选 Work scope 的限额、known/consumed、版本；
...
- Store 并发测试：Gate 最后一次授权、预算 CAS、全局 Event 游标、重启读回。
