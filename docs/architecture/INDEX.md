# Architecture Index

<!-- AGENT-HARNESS:BEGIN INDEX -->
| ID | 标题 | 文件 |
|---|---|---|
| DESIGN-001-m2-git-environment-validation | DESIGN-001：当前工作目录、Git 快照与验证闭环 | [DESIGN-001-m2-git-environment-validation.md](DESIGN-001-m2-git-environment-validation.md) |
| DESIGN-002-m3-codex-adapter | DESIGN-002：M3 Codex CLI Adapter | [DESIGN-002-m3-codex-adapter.md](DESIGN-002-m3-codex-adapter.md) |
| DESIGN-003-m4-claude-review | DESIGN-003：M4 Claude Adapter 与独立 Review | [DESIGN-003-m4-claude-review.md](DESIGN-003-m4-claude-review.md) |
| DESIGN-004-m5-control-daemon | DESIGN-004：确定性控制面、项目隔离与持久后台执行 | [DESIGN-004-m5-control-daemon.md](DESIGN-004-m5-control-daemon.md) |
| DESIGN-005-m6-finalization-release | DESIGN-005：M6 最终闭环、Benchmark 与发布 | [DESIGN-005-m6-finalization-release.md](DESIGN-005-m6-finalization-release.md) |
| DESIGN-006-cobra-cli | DESIGN-006：Cobra CLI 迁移设计 | [DESIGN-006-cobra-cli.md](DESIGN-006-cobra-cli.md) |
| THREAT-MODEL-v0.1 | xgoal v0.1 威胁模型 | [THREAT-MODEL-v0.1.md](THREAT-MODEL-v0.1.md) |
<!-- AGENT-HARNESS:END INDEX -->
