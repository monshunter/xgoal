...
- Evidence：`WorkPacket.Validate` 只要求 Network/Secret 非空，不校验允许值，也不拒绝 `git_push=true` 或 `production=true`；`AgentEvent.Validate` 不拒绝负 Token/Cost 或缺失 Command argv。嵌入 Schema 对这些字段有 Enum、Const、Minimum 与 Required 约束。
...
- Evidence：Workspace Provider、Bootstrap Command、Scope Policy、Review/Policy/Budget/Report 未校验；Validator 可缺少 Phase、正 Timeout、Expected Exit Code；非 Fake Agent 的 Adapter 专属 Sandbox/PermissionMode 也未收敛。当前 README 把该入口描述为严格配置校验。
