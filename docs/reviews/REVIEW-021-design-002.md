...
- Probe 明确区分无费用 Passive 与显式 Active；Cost 不可得时记录 unknown，不把 token 估算伪装成实际金额。
- 真实写入验收以 M2 Patch/Scope/Replay/Validator 为最终 owner，符合 Claim 与 Evidence 分层。

## Notes

- 当前 Codex JSONL 的具体 Item 子类型允许向前兼容；只有 Session、最终 Agent Result 和 Usage 等进入控制决策的字段使用严格已知合同。
- M5 接入 Kernel 时必须把协议损坏/no-progress 分类传入 SessionPolicy，并把 Active Probe 用量纳入统一 Budget Store。
