...
包括 Codex Passive/Active Probe、`codex exec --json --output-schema`、角色 Sandbox、原始事件脱敏与不可变引用、结构化结果校验、进程组取消、Session Resume 安全绑定、fixture contract 和真实 CLI smoke；不包括 Claude Adapter/Reviewer、完整 Reconcile/Gate、Daemon/API、Final Report 与 Benchmark。
...
- [x] 2.3 实现无模型调用的 Passive Probe 与显式、受正超时约束的 Active Contract Probe
