...
- [x] 2.1 实现主工作目录独占会话、干净基线准入和用户 HEAD/index 完整性核对
- [x] 2.2 用独立临时 index 捕获当前目录 Tree 和完整 Patch，排除 Git 元数据及 xgoal 状态
- [x] 2.3 替换工作区状态与制品布局，保留旧历史可读并拒绝不安全的旧运行恢复

## Phase 3：贯通验证、审查与结果交付

- [x] 3.1 在当前目录执行 Validator 与独立 Reviewer，并核对执行前后的受验收 Tree
- [x] 3.2 从已验证 Tree 创建私有集成 Commit/Ref，保留用户 HEAD/index 和当前目录结果
- [x] 3.3 实现失败、中断和外部编辑的现场保留、等待及明确恢复去向
- [x] 3.4 对齐配置、初始化、CLI 状态、README 和操作文档，移除实际 worktree 创建入口

## Phase 4：验证与收口

- [x] 4.1 覆盖多 Work 顺序执行、完整文件差异、元数据保护、验证变更和历史兼容的定向测试
- [x] 4.2 以真实 CLI/daemon 场景证明工作目录结果、Git worktree 列表不变及最终 Evidence/Report 绑定
- [x] 4.3 完成相关回归、独立 Change Review、制品对账与原子提交
