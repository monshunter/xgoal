# PLAN-006：实现 M5 Reconcile、Gate 与 Daemon

## 目标

在 M1–M4 的持久状态、验证、Agent 与 Review 基础上，建立确定性的失败归因与防机械重试、有限 Human Gate 授权，以及由单写 Daemon 承载的 Unix Socket API、状态流和进程恢复。

## 范围

包括 Failure Class/Fingerprint、Material Progress、Reconcile/Replan 决策；Policy 与有限授权消费；SQLite 迁移与全局事件游标；Unix Socket HTTP/JSON API、NDJSON 状态流、Daemon 生命周期和 Worker 恢复；对应 CLI 与故障注入门禁。最终报告、Benchmark、Threat Model 与发布清单属于 M6。

## Phase 1：冻结控制面合同

关联：[技术 SPEC 18–20、23、27 与 M5](../../xgoal-technical-spec-v0.1.md#18-reconcile-与防机械重试)

- [x] 1.1 冻结 Failure Fingerprint、Material Progress、Reconcile/Replan 与 Policy/Gate 的确定性合同
- [x] 1.2 冻结 SQLite、Unix Socket API、NDJSON 游标、Daemon 单写和进程恢复设计并通过 Design Review

## Phase 2：实现 Reconcile 与 Policy/Gate
...
- [x] 2.3 实现 Failure/Decision、Gate、Worker 与全局 Event 游标的 SQLite 持久化
...
- [x] 4.1 通过同指纹无进展、Gate 越权/过期/超次与策略 fail-closed 安全矩阵
